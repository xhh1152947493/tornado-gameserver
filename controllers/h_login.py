...
class CGuestLoginHandler(CBaseHandler):
	def prepare(self):
		pass
	...
	def _new_user_login(self, conn, conn_redis, imei):
		...
		logger.info("guest new user login success. uid:%d imei:%s", uid, imei)
	...
	def _login_by_guest(self, imei):
		...
		logger.info("guest old user login success. uid:%d imei:%s", uid, imei)
		return self.write_json(error.OK, bs)

# class CWeChatLoginHandler(CBaseHandler):
# 	def prepare(self):
# 		pass
#
# 	def get(self):
